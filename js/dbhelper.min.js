...
static updateFavoriteStatus(restaurantId,isFavorite){fetch(`${DBHelper.DATABASE_RESTAURANT_URL}/${restaurantId}/?is_favorite=${isFavorite}`,{method:'PUT'}).then(()=>{console.log('successful change');console.log(isFavorite);this.dbPromise().then(db=>{const tx=db.transaction('restaurants','readwrite');const restaurantsStore=tx.objectStore('restaurants');return restaurantsStore.get(restaurantId).then(restaurant=>{console.log(restaurant);restaurant.is_favorite=isFavorite;console.log(restaurant);restaurantsStore.put(restaurant)})})})}
